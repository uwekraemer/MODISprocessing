...
from utils.utilities import ensureTrailingSlash
from nasa.modis.seadas_processing.conf.paths import modisL3_TSM_UTM_QLPath


def printUsage():
    print("Usage: ", argv[0], "<year>")
    print("where year is a string representing the year for data upload,")
    print("e.g. 2013")
...
for a in range(listSize):
    for item in srcList:
        if not item.startswith('cb_ns_' + param + '_') or not item.find(_year) > 1 or not item.endswith('_eo_bc.png'):
