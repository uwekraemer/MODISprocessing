...
def printUsage():
    print("Usage: running_weekly_mean year num_days")
...
try:
    ...
    print("year parameter must be of type integer!")
    printUsage()
...
def make_doy_sequences():
    _year_seq = []
    last_index_of_year = num_days_in_year - _num_days_in_l3 + 2

    for day_index in range(1, last_index_of_year, 1):
        _l3_seq_day_range = list(range(day_index, day_index + _num_days_in_l3, 1))
        _l3_seq = []
        for i in list(range(_num_days_in_l3)):
            _l3_seq.append(str(_l3_seq_day_range[i]).zfill(3))
        _year_seq.append(_l3_seq)
    return _year_seq
...
_year_seq = make_doy_sequences()

for _seq_ind in range(len(_year_seq)):
    _seq = _year_seq[_seq_ind]
    print("New request:", _seq)
    _output_product_name = 'A' + str(_year) + _seq[0] + '_' + str(_year) + _seq[-1] + '_L3_' + _l3_region + '_' + _l3_resolution + 'km' + _l3_out_prod_ext
