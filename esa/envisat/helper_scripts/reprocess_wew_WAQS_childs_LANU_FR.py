...
print("\n**************************************************")
print(" Script \'reprocess_wew_WAQS_childs_LANU_FR.py\' at work... ")
...
# Verzeichnisse
baseDir = '/EOData2/related/GeoInfoServices/LANU/MERIS/FR/L1/'
smileCorrSrcDir  = baseDir
smileCorrDestDir = '/fs14/EOservices/InputPool/MERIS/RR/waqs_child_temp/smile_corrected/'

wewSrcDir = smileCorrDestDir
wewDestDir = '/EOData2/related/GeoInfoServices/LANU/MERIS/FR/WAQS-WeW/daily/'
...
for date_int in range(int(start_date), int(end_date)+1):
    ...
    for a in range(list_size):
        for item in src_list:
            if not item.startswith('MER') or item.startswith('MER_RR__0') or item.startswith('MER_RR__2') or item.endswith('.data') or item.find(date)<0:
    ...
    for meris_file in src_list:    
        ...
        wew_output_filename= wewDestDir + "FR_" + acquisition_time + "_" + orbit + "_fw.dim"
