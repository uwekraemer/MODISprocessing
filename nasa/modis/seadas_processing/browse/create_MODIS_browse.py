...
from nasa.modis.seadas_processing.conf.paths import seadasBinDir, imageMagickBinDir, modisGEOBasePath, modisL1B_LACBasePath, modisBrowseBasePath


def printUsage():
    print('Usage: create_MODIS_browse.py date')
...
for a in range(listSize):
    for item in srcList:
        ...
        if not item.startswith('A' + _year +str(_doy)) or not item.endswith('.L1B_LAC.x.hdf'):
            _remove_from_list = True

        if item.endswith('.L1B_LAC.x.hdf') or item.endswith('.L1B_LAC.x.hdf.bz2'):
            _remove_from_list = False
        else:
            _remove_from_list = True
...
def process_modis_browse(l1b_productPath):
    ...
    browse_product  = basename(l1b_productPath.replace('L1B_LAC.x.hdf', 'ppm'))
...
for item in srcList:
    print('\n', item)
    zipped = False
    if item.endswith('.L1B_LAC.x.hdf'):
        zipped = False
        l1b_product = item
    elif item.endswith('.L1B_LAC.x.hdf.bz2'):
        zipped = True
        l1b_product = item[:-4]

    jpg_browse_file = modisL1B_LACPath + l1b_product.replace('L1B_LAC.x.hdf', 'jpg')
